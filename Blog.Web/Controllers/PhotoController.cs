        ...
        public async Task<ActionResult<int>> Delete(int photoId)
        {
            ...
            if (foundPhoto != null)
            {
                if (foundPhoto.ApplicationUserId == applicationUserId)
                {
                    ...
                    if (usedInBlogs) return BadRequest("Cannot remove photo as it is used in published blog(s).");

                    var deleteResult = await _photoService.DeletePhotoAsync(foundPhoto.PublicId);

                    if (deleteResult.Error != null) return BadRequest(deleteResult.Error.Message);

                    var affetedRows = await _photoRepository.DeleteAsync(foundPhoto.PhotoId);

                    return Ok(affetedRows);
                }
                else
                {
